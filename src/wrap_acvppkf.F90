...
DO NSTEP=1,NSTEPMAX
  WRITE(*,*)"NSTEP:",NSTEP
!$OMP PARALLEL DO PRIVATE(BLK_SIZE)
  DO K=1,NBLKS
    IF(K==NBLKS)THEN
      BLK_SIZE = LAST_BLK_NPROMA
    ELSE
...
DO NSTEP=1,NSTEPMAX
  DO K=1,NBLKS
    ...
    DO J=0,KLEV
      DO I=1,BLK_SIZE
        IF(ORI_PDIFCQ(I+(K-1)*NPROMA,J,NSTEP) /= PDIFCQ(I,J,K,NSTEP))WRITE(*,*)"ERROR ORI_PDIFCQ",ORI_PDIFCQ(I+(K-1)*BLK_SIZE,J,NSTEP),PDIFCQ(I,J,K,NSTEP)
        IF(ORI_PDIFCS(I+(K-1)*NPROMA,J,NSTEP) /= PDIFCS(I,J,K,NSTEP))WRITE(*,*)"ERROR ORI_PDIFCS"
        IF(ORI_PFCCQL(I+(K-1)*NPROMA,J,NSTEP) /= PFCCQL(I,J,K,NSTEP))WRITE(*,*)"ERROR ORI_PFCCQL"
        IF(ORI_PFCCQN(I+(K-1)*NPROMA,J,NSTEP) /= PFCCQN(I,J,K,NSTEP))WRITE(*,*)"ERROR ORI_PFCCQN"
        IF(ORI_PPRODTH(I+(K-1)*NPROMA,J,NSTEP) /= PPRODTH(I,J,K,NSTEP))WRITE(*,*)"ERROR ORI_PPRODTH"
        IF(J>0)THEN
          IF(ORI_PQCPP(I+(K-1)*NPROMA,J,NSTEP) /= PQCPP(I,J,K,NSTEP))WRITE(*,*)"ERROR ORI_PQCPP"
          IF(ORI_PNEBPP(I+(K-1)*NPROMA,J,NSTEP) /= PNEBPP(I,J,K,NSTEP))WRITE(*,*)"ERROR ORI_PNEBPP"
          IF(ORI_KNLAB(I+(K-1)*NPROMA,J,NSTEP) /= KNLAB(I,J,K,NSTEP))WRITE(*,*)"ERROR ORI_KNLAB"
          IF(ORI_KNND(I+(K-1)*NPROMA,NSTEP) /= KNND(I,K,NSTEP))WRITE(*,*)"ERROR ORI_KNND"
        ENDIF
      ENDDO
    ENDDO
  ENDDO
